...
//  and returns result, a complex number, in det.
	...
	return u[0 * 3 + 0] * (u[1 * 3 + 1] * u[2 * 3 + 2] - u[1 * 3 + 2] * u[2 * 3 + 1]) 
		 + u[0 * 3 + 1] * (u[1 * 3 + 2] * u[2 * 3 + 0] - u[1 * 3 + 0] * u[2 * 3 + 2])
	     + u[0 * 3 + 2] * (u[1 * 3 + 0] * u[2 * 3 + 1] - u[1 * 3 + 1] * u[2 * 3 + 0]);
...
//  More explanation below.
    ...
    //  First component:
	x_SU3[2 * 3 + 0] = (u_new_conj[1] * v_new_conj[2] 
					  - u_new_conj[2] * v_new_conj[1]);
	
    //  Second component:
	x_SU3[2 * 3 + 1] = (u_new_conj[2] * v_new_conj[0] 
					  - u_new_conj[0] * v_new_conj[2]);

    //  Third component:
	x_SU3[2 * 3 + 2] = (u_new_conj[0] * v_new_conj[1] 
					  - u_new_conj[1] * v_new_conj[0]);
...
void SU3_decompose_algebra(double complex * a, double * a_components){ 
	...
	a_components[8] = (creal(a[0 * 3 + 0]) + creal(a[1 * 3 + 1]) 
						- 2.0 * creal(a[2 * 3 + 2])) / pow(3.0, 0.5);
}